        ...
        <section className="relative py-24 bg-gray-50 dark:bg-slate-800" id="testimonials">
          <div className="container">
            <div className="grid grid-cols-1 pb-8 text-center">
              <h3 className="mb-6 text-2xl md:text-3xl font-semibold">Testimonials</h3>
              <p className="text-gray-600 dark:text-gray-300 max-w-xl mx-auto text-sm md:text-base">{portfolioData.user.about.subTitle}</p>
            </div>
            <Slider {...settings}>
              {portfolioData.user.testimonials
                .filter((item) => item.enabled)
                .map((item) => (
                  <div key={item._id} className="px-4">
                    <div className="customer-testimonial bg-white dark:bg-slate-900 rounded shadow-md p-6">
                      <i className="mdi mdi-format-quote-open text-amber-500 text-4xl"></i>
                      <p className="text-slate-400 text-[15px] py-4">{item.review}</p>
