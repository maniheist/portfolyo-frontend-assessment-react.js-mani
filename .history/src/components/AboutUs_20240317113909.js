...
export default function AboutUs() {
  ...
  <div className="grid grid-cols-1 pb-8 text-center">
          ...
          style={{ transform: isHovered ? 'translateY(-5px)' : 'none', transition: 'transform 0.3s ease' }}
      ...
      .map((item, index) => {
        return (
          <div
            ...
            key={index}
          >
            <div className="flex items-center justify-center h-12 w-12 bg-amber-500/10 group-hover:bg-amber-500 text-amber-500 group-hover:text-white rounded-full me-5">
              <img
                src={item.image.url}
                alt={item.name}
                className="w-8 h-8"
