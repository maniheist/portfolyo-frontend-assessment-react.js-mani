              ...
              {portfolioData.user.timeline.map((item, index) => {
                ...
                return (
                  <div key={item._id} className="relative overflow-hidden">
                    ...
                    <span className="absolute top-0 left-0 ml-2 mt-1 text-xs font-medium text-amber-500">{item.sequence}</span>
                    {item.forEducation && (
                      <span className="absolute top-0 right-12 mr-2 mt-1 text-xs font-medium text-green-500">For Education</span>
                    )}
                  </div>
                );
              })}
