...
export default function Switcher() {
    ...
    return (
        <>
            <div className="fixed top-1/4 right-5 z-50">
  <span className="relative inline-block">
    ...
    />
    <label
      className="label bg-gray-300 dark:bg-gray-800 shadow dark:shadow-gray-800 cursor-pointer rounded-full flex justify-between items-center p-1 w-14 h-8 transition-colors duration-300 ease-in-out"
      htmlFor="chk"
    >
      <Unicons.UilMoon width={16} className="text-[20px] text-yellow-500" />
      <Unicons.UilSun width={16} className="text-[20px] text-yellow-500" />
      <span className="ball bg-white dark:bg-gray-700 rounded-full absolute top-[2px] left-[2px] w-7 h-7 transition-transform duration-300 ease-in-out transform translate-x-[100%]"></span>
    </label>
  </span>
</div>
