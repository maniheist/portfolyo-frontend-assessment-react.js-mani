  ...
  return (
    <>
      {portfolioData.success === true && (
        <section className="py-16 bg-gradient-to-b from-gray-100 to-gray-200 dark:from-slate-800 dark:to-slate-900" id="service">
              ...
              {portfolioData.user.services.map((item, index) => {
                return (
                  <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden" key={index}>
                    <img src={item.image.url} alt={item.name} className="w-full h-40 object-cover object-center" />
                    <div className="p-6">
                      <h4 className="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-100 mb-2">{item.name}</h4>
                      <p className="text-sm md:text-base text-gray-600 dark:text-gray-300 mb-4">{item.desc}</p>
